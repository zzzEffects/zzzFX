//! Builds the zzzSpriteSheetReader OpenFX plugin and bundles it.
...
pub fn command() -> clap::Command {
    clap::Command::new("build-zzzsprite-sheet-ofx-plugin")
        .about("Builds and bundles the zzzSpriteSheetReader OpenFX plugin.")
        .arg(
        ...
        .arg(
            clap::Arg::new("destdir")
                ...
                .default_value(
                    workspace_dir()
                        .plus_iter(["crates", "zzz-sprite-sheet-openfx", "build"])
                        .as_os_str()
                        .to_owned(),
                ),
        )
...
fn get_info_plist() -> plist::Value {
    let cargo_toml_path =
        workspace_dir().plus_iter(["crates", "zzz-sprite-sheet-openfx", "Cargo.toml"]);
    ...
    info_plist_contents.insert(
        ...
        plist::Value::from("com.example.zzzspritesheetreader"),
    );
    info_plist_contents.insert(
    ...
    info_plist_contents.insert(
        ...
        plist::Value::from("zzzSpriteSheetReader Plugin"),
    );
    info_plist_contents.insert(
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    println!(
        "Building zzzSpriteSheetReader OFX plugin for target {}",
        target.target_triple
    );

    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=zzz-sprite-sheet-openfx-plugin"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path = target_dir_path
        .plus(target.library_prefix.to_owned() + "zzz_sprite_sheet_openfx_plugin");
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let (built_library_path, ofx_architecture) = if args.get_flag("macos-universal") {
        ...
        let target = TARGETS
            ...
            .unwrap_or_else(|| {
                panic!("Your target \"{}\" is not supported", target_triple)
            });
        (
    ...
    let plugin_bundle_path =
        output_dir.plus_iter(["zzzSpriteSheetReader.ofx.bundle", "Contents"]);
    let plugin_bin_path =
        plugin_bundle_path.plus_iter([ofx_architecture, "zzzSpriteSheetReader.ofx"]);
