//! Builds the zzzRepeater OpenFX plugin and bundles it.
...
pub fn command() -> clap::Command {
    clap::Command::new("build-zzzrepeater-ofx-plugin")
        .about("Builds and bundles the zzzRepeater OpenFX plugin.")
        .arg(
        ...
        .arg(
            clap::Arg::new("destdir")
                ...
                .default_value(
                    workspace_dir()
                        .plus_iter(["crates", "zzz-repeater-openfx", "build"])
                        .as_os_str()
                        .to_owned(),
                ),
        )
...
fn get_info_plist() -> plist::Value {
    let cargo_toml_path = workspace_dir().plus_iter(["crates", "zzz-repeater-openfx", "Cargo.toml"]);
    ...
    info_plist_contents.insert("CFBundleIdentifier".to_string(), plist::Value::from("com.example.zzzrepeater"));
    ...
    info_plist_contents.insert("NSHumanReadableCopyright".to_string(), plist::Value::from("zzzRepeater Plugin"));
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    println!("Building zzzRepeater OFX plugin for target {}", target.target_triple);

    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=zzz-repeater-openfx-plugin"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path = target_dir_path.plus(target.library_prefix.to_owned() + "zzz_repeater_openfx_plugin");
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let plugin_bundle_path = output_dir.plus_iter(["zzzRepeater.ofx.bundle", "Contents"]);
    let plugin_bin_path = plugin_bundle_path.plus_iter([ofx_architecture, "zzzRepeater.ofx"]);
