//! Builds the zzzStroke OpenFX plugin and bundles it.
...
pub fn command() -> clap::Command {
    clap::Command::new("build-zzzstroke-ofx-plugin")
        .about("Builds and bundles the zzzStroke OpenFX plugin.")
        .arg(
        ...
        .arg(
            clap::Arg::new("destdir")
                ...
                .default_value(
                    workspace_dir()
                        .plus_iter(["crates", "zzz-stroke-openfx", "build"])
                        .as_os_str()
                        .to_owned(),
                ),
        )
...
fn get_info_plist() -> plist::Value {
    let cargo_toml_path = workspace_dir().plus_iter(["crates", "zzz-stroke-openfx", "Cargo.toml"]);
    ...
    info_plist_contents.insert("CFBundleIdentifier".to_string(), plist::Value::from("com.example.zzzstroke"));
    ...
    info_plist_contents.insert("NSHumanReadableCopyright".to_string(), plist::Value::from("zzzStroke Plugin"));
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    println!("Building zzzStroke OFX plugin for target {}", target.target_triple);

    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=zzz-stroke-openfx-plugin"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path = target_dir_path.plus(target.library_prefix.to_owned() + "zzz_stroke_openfx_plugin");
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let plugin_bundle_path = output_dir.plus_iter(["zzzStroke.ofx.bundle", "Contents"]);
    let plugin_bin_path = plugin_bundle_path.plus_iter([ofx_architecture, "zzzStroke.ofx"]);
