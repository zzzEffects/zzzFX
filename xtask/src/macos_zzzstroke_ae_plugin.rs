//! Builds and bundles the ZZZ Stroke After Effects plugin for macOS.
...
pub fn command() -> clap::Command {
    clap::Command::new("macos-zzzstroke-ae-plugin")
        .about("Builds and bundles the ZZZ Stroke After Effects plugin for macOS.")
        .arg(
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<(PathBuf, PathBuf)> {
    println!("Building ZZZ Stroke AE plugin for target {}", target.target_triple);

    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=zzz-stroke-ae-plugin"),
        String::from("--target"),
    ...
    built_library_path.push(target.library_prefix.to_owned() + "zzz_stroke_ae_plugin");
    built_library_path.set_extension(target.library_extension);

    let built_rsrc_path = target_dir_path.plus("zzz-stroke-ae-plugin.rsrc");
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let plugin_dir_path = build_dir_path.plus("ZZZStroke.plugin");
    ...
    fs::copy(built_library_path, macos_dir_path.plus("ZZZStroke"))?;
    fs::copy(built_rsrc_path, resources_dir_path.plus("ZZZStroke.rsrc"))?;
