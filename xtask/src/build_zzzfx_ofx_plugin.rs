    ...
    clap::Command::new("build-zzzfx-ofx-plugin")
        ...
        .arg(
            clap::Arg::new("destdir")
                ...
                .default_value(
                    workspace_dir()
                        .plus_iter(["crates", "zzzfx-ofx", "build"])
                        .as_os_str()
                        .to_owned(),
                ),
        )
...
fn get_info_plist() -> plist::Value {
    let cargo_toml_path = workspace_dir().plus_iter(["crates", "zzzfx-ofx", "Cargo.toml"]);
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    ...
    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=zzzfx-ofx-plugin"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path = target_dir_path.plus(target.library_prefix.to_owned() + "zzzfx_ofx_plugin");
