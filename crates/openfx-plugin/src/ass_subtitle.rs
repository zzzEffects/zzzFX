...
use zzzfx::{
    AssSubtitle, AssSubtitleFullSettings,
...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<AssSubtitleFullSettings>,
    strings: StringCache<AssSubtitleFullSettings>,
    menu_item_strings: MenuItemCache<AssSubtitleFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"io.github.zzzEffects:AssSubtitle".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<AssSubtitleFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = AssSubtitleFullSettings::default();
...
unsafe fn action_render(
    ...
    let mut settings = AssSubtitleFullSettings::default();
    ...
    let ss: AssSubtitle = (&settings).into();
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut AssSubtitleFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;

    let find_id = |name: &str| -> OfxResult<SettingID<AssSubtitleFullSettings>> {
