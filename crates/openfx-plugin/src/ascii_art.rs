...
use zzzfx::settings::{SettingKind, TrKey};
use zzzfx::{
...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<AsciiArtFullSettings>,
    strings: StringCache<AsciiArtFullSettings>,
    menu_item_strings: MenuItemCache<AsciiArtFullSettings>,
...
pub fn get_plugin() -> *const OfxPlugin {
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"io.github.zzzEffects:AsciiArt".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<AsciiArtFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = AsciiArtFullSettings::default();
