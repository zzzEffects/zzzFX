...
use zzzfx::svg_display::CachedSvg;
use zzzfx::{
    SvgDisplay, SvgDisplayFullSettings,
...
fn is_native_grouped_name(name: &str) -> bool {
    matches!(name, "position_x" | "position_y" | "file_path" | "file_data")
}
...
const INSTANCE_MAGIC: u64 = 0x5ADE_5ADE_5ADE_5ADE;

struct InstanceData {
    magic: u64,
    file_path: String,
    svg_bytes: Vec<u8>,
    cached_svg: Option<CachedSvg>,
}
...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<SvgDisplayFullSettings>,
    strings: StringCache<SvgDisplayFullSettings>,
    menu_item_strings: MenuItemCache<SvgDisplayFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"io.github.zzzEffects:SvgDisplay".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<SvgDisplayFullSettings>::new();
...
unsafe fn action_describe(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    ps(ep, kOfxPropLabel.as_ptr(), 0, i18n::tr_cstr(TrKey::EffectSvgDisplayName).as_ptr()).ofx_ok()?;
    ps(ep, kOfxImageEffectPluginPropGrouping.as_ptr(), 0, c"zzzFX".as_ptr()).ofx_ok()?;
    ps(ep, kOfxPropPluginDescription.as_ptr(), 0, i18n::tr_cstr(TrKey::EffectSvgDisplayDesc).as_ptr()).ofx_ok()?;
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = SvgDisplayFullSettings::default();
    ...
    // --- Native PushButton: fileSelect ---
    {
        ...
        ps(pp, kOfxPropLabel.as_ptr(), 0, i18n::tr_cstr(TrKey::NativeSelectSvgFile).as_ptr()).ofx_ok()?;
        ps(pp, kOfxParamPropHint.as_ptr(), 0, i18n::tr_cstr(TrKey::NativeSelectSvgFileHint).as_ptr()).ofx_ok()?;
    ...
    // --- Native RGBA: backgroundColor (default transparent) ---
    {
        ...
        ps(pp, kOfxPropLabel.as_ptr(), 0, i18n::tr_cstr(TrKey::NativeSvgBackgroundColor).as_ptr()).ofx_ok()?;
        ps(pp, kOfxParamPropHint.as_ptr(), 0, i18n::tr_cstr(TrKey::NativeSvgBackgroundColorHint).as_ptr()).ofx_ok()?;
    ...
    // --- Interleave native Double2D: Position (before Rotation) ---
    ...
    for desc in d.settings_list.setting_descriptors.iter() {
        if !defined_position && desc.id.name == "position_x" {
            define_native_double2d(
                ...
                i18n::tr_cstr(TrKey::NativeSvgPosition),
...
unsafe fn action_instance_changed(
    ...
    if CStr::from_ptr(target_type) == kOfxTypeParameter {
        ...
        if FILE_SELECT_PARAM == CStr::from_ptr(target_name) {
            ...
            let svg_bytes = std::fs::read(&path).map_err(|_| OfxStat::kOfxStatFailed)?;
...
unsafe fn action_render(
    ...
    let mut settings = SvgDisplayFullSettings::default();
    let (bg_r, bg_g, bg_b, bg_a, pos_x, pos_y) = apply_params(param_set, time, &mut settings)?;
    settings.rotation = ofx_angle_to_renderer(settings.rotation as f64) as f32;
    let core_settings: SvgDisplay = (&settings).into();
    ...
    if let Some(idata) = idata {
        ...
        if !idata.svg_bytes.is_empty() {
            let new_cache = zzzfx::svg_display::render_svg(
                &idata.svg_bytes,
                idata.cached_svg.as_ref(),
                &core_settings,
                pos_x,
                pos_y,
                &mut dst_buf,
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut SvgDisplayFullSettings,
) -> OfxResult<(f32, f32, f32, f32, f32, f32)> {
    let d = data()?;
    let su = &d.suites;
    ...
    let find_id = |name: &str| -> OfxResult<SettingID<SvgDisplayFullSettings>> {
    ...
    // Read native Double2D: position (Y flipped: OFX 0=bottom → pixmap 0=top)
    let (pos_x, pos_y) = {
        let mut p: OfxParamHandle = ptr::null_mut();
    ...
    // Read generic params (skip position_x/y since they come from Double2D)
    ...
    Ok((bg_r, bg_g, bg_b, bg_a, pos_x, pos_y))
}
