...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<MultiToneFullSettings>,
    strings: StringCache<MultiToneFullSettings>,
    menu_item_strings: MenuItemCache<MultiToneFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"io.github.zzzEffects:MultiTone".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<MultiToneFullSettings>::new();
...
unsafe fn action_describe(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    ps(
        ...
        0,
        i18n::tr_cstr(TrKey::EffectMultiToneName).as_ptr(),
    )
    .ofx_ok()?;
    ps(
        ep,
    ...
    ps(
        ...
        0,
        i18n::tr_cstr(TrKey::EffectMultiToneDesc).as_ptr(),
    )
    .ofx_ok()?;
    ps(
        ep,
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let ps = su.property_suite.propSetString.ok_or(OfxStat::kOfxStatFailed)?;
    let defaults = MultiToneFullSettings::default();
...
unsafe fn action_is_identity(
    ...
    let mut settings = MultiToneFullSettings::default();
    apply_params(param_set, time, &mut settings)?;
    let multitone: MultiTone = (&settings).into();

    if multitone.is_identity() {
...
unsafe fn action_render(
    ...
    RENDER_BUFS.with(|cell| {
        if let Ok(mut bufs) = cell.try_borrow_mut() {
            ...
            multitone.apply_effect(src_buf, dst_buf, width, height);
            ...
            multitone.apply_effect(&src_buf, &mut dst_buf, width, height);
            copy_u8_to_output(&dst_buf, dp, d_stride, width, height, row_bytes_u8, depth);
        }
    });
