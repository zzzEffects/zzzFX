...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ChromaKeyFullSettings>,
    strings: StringCache<ChromaKeyFullSettings>,
    menu_item_strings: MenuItemCache<ChromaKeyFullSettings>,
}
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ChromaKeyFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = ChromaKeyFullSettings::default();
...
unsafe fn action_render(effect: OfxImageEffectHandle, inArgs: OfxPropertySetHandle) -> OfxResult<()> {
    ...
    let mut settings = ChromaKeyFullSettings::default();
    apply_params(param_set, time, &mut settings)?;

    let chroma_key: ChromaKey = (&settings).into();
