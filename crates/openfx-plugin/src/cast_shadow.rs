...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<CastShadowFullSettings>,
    strings: StringCache<CastShadowFullSettings>,
    menu_item_strings: MenuItemCache<CastShadowFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"io.github.zzzEffects:CastShadow".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<CastShadowFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = CastShadowFullSettings::default();
...
unsafe fn action_is_identity(
    ...
    let mut settings = CastShadowFullSettings::default();
    apply_params(param_set, time, &mut settings)?;
    let shadow: CastShadow = (&settings).into();
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut CastShadowFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;
    let pgh = su.parameter_suite.paramGetHandle.ok_or(OfxStat::kOfxStatFailed)?;
