...
use example_effect::{
    CompositorLayer, ZzzRepeater, ZzzRepeaterFullSettings,
    settings::{
...
struct Plugin {
    settings: SettingsList<ZzzRepeaterFullSettings>,
}

impl Default for Plugin {
    fn default() -> Self {
        Self {
            settings: SettingsList::<ZzzRepeaterFullSettings>::new(),
        }
    }
}
...
impl AdobePluginGlobal for Plugin {
    fn params_setup(
        ...
        Self::map_params(
            ...
            &ZzzRepeaterFullSettings::default(),
            &ZzzRepeaterFullSettings::legacy_value(),
        )?;
...
impl Plugin {
    ...
    fn about(&self, _in_data: InData, mut out_data: OutData) -> Result<(), Error> {
        const DESCRIPTION: &str = "Keyframe-driven repeater effect. Each keyframe on Time Offset triggers a repeat layer. Supports 22 blend modes, position, and rotation.";
        out_data.set_return_msg(
            format!(
                "ZZZ Repeater {}.{}.{}\r\r{DESCRIPTION}",
    ...
    fn do_render(
        ...
        let full_settings: ZzzRepeaterFullSettings = self.apply_settings(params)?;
        ...
        let src_buf_raw = in_layer.buffer();
        let dst_buf_raw = out_layer.buffer_mut();

        // Copy from strided source to contiguous buffer
        let mut src_buf = vec![0u8; total];
        for y in 0..height {
            let src_offset = y * src_stride;
            unsafe {
                std::ptr::copy_nonoverlapping(
                    src_buf_raw.as_ptr().add(src_offset),
        ...
        // Single-layer render: treat source as a single compositor layer
        // Note: Keyframe-driven repeat layers are not supported in the AE wrapper
        // due to AE API limitations for multi-time image fetching.
        // The OFX plugin supports the full feature set.
        ...
        // Copy result back to strided destination buffer
        for y in 0..height {
            let dst_offset = y * dst_stride;
            unsafe {
                std::ptr::copy_nonoverlapping(
                    dst_buf.as_ptr().add(y * row_bytes),
                    dst_buf_raw.as_mut_ptr().add(dst_offset),
                    row_bytes,
                );
            }
        }
    ...
    // -----------------------------------------------------------------------
    // Parameter mapping (generic)
    // -----------------------------------------------------------------------

    fn update_controls_disabled(
        params: &mut Parameters<ParamID>,
        descriptors: &[SettingDescriptor<ZzzRepeaterFullSettings>],
        enabled: bool,
    ) -> Result<(), Error> {
    ...
    fn map_params(
        ...
        legacy_default_settings: &ZzzRepeaterFullSettings,
    ) -> Result<(), Error> {
        fn get_defaults<T: example_effect::settings::SettingField + 'static>(
            defaults: &ZzzRepeaterFullSettings,
            legacy_defaults: &ZzzRepeaterFullSettings,
    ...
    fn apply_settings(
        &self,
        params: &mut Parameters<ParamID>,
    ) -> Result<ZzzRepeaterFullSettings, Error> {
        let mut settings = ZzzRepeaterFullSettings::default();

        fn apply_settings_list(
            descriptors: &[SettingDescriptor<ZzzRepeaterFullSettings>],
            params: &mut Parameters<ParamID>,
            settings: &mut ZzzRepeaterFullSettings,
        ) -> Result<(), Error> {
            for descriptor in descriptors {
                match &descriptor.kind {
                    SettingKind::Enumeration { options, .. } => {
