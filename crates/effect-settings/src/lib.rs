...
//! This lets various UIs and effect plugins query the set of settings and display them
//! in their preferred format without duplicating code.

pub mod i18n_keys;
pub use i18n_keys::{ExTrKey, I18nKey, TrKey};
...
macro_rules! setting_id {
    ($name:expr, $($field_path:ident).+) => {
        $crate::SettingID::new(
            $name,
            |settings| $crate::SettingField::upcast(settings.$($field_path).+),
            |settings, value| {
                settings.$($field_path).+ = $crate::SettingField::downcast(&value).ok_or_else(|| $crate::GetSetFieldError::TypeMismatch {
...
#[derive(Debug, Clone)]
pub struct MenuItem<K: I18nKey = TrKey> {
    pub label_key: K,
    pub description_key: Option<K>,
    pub index: u32,
}

/// All of the types a setting can take.
#[derive(Debug, Clone)]
...
pub trait Settings: Default {
    /// The i18n key type used for labels and descriptions in this setting's
    /// descriptors. zzzFX plugins use [`TrKey`]; example-effect plugins use [`ExTrKey`].
    type Key: I18nKey;
...
/// A single setting descriptor, including label, description, kind, and ID.
#[derive(Debug, Clone)]
...
/// Convenience trait for asserting the "shape" of the JSON we're parsing.
pub(crate) trait GetAndExpect {
...
impl<T: Settings> SettingsList<T> {
    ...
    /// Parse settings from a given JSON string and return a new settings struct.
...
/// Iterator over all setting descriptors in depth-first order.
