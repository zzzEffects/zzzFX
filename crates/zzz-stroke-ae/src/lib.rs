...
struct Plugin {
    settings: SettingsList<ZzzStrokeFullSettings>,
}

impl Default for Plugin {
    fn default() -> Self {
        Self {
            settings: SettingsList::<ZzzStrokeFullSettings>::new(),
        }
    }
}
...
impl AdobePluginGlobal for Plugin {
    fn params_setup(
        ...
        Self::map_params(
            ...
            &ZzzStrokeFullSettings::default(),
            &ZzzStrokeFullSettings::legacy_value(),
        )?;
...
impl Plugin {
    ...
    fn about(&self, _in_data: InData, mut out_data: OutData) -> Result<(), Error> {
        const DESCRIPTION: &str = "Alpha-channel stroke effect with distance transform, multiple fill modes, and blend options.";
        out_data.set_return_msg(
            format!(
                "ZZZ Stroke {}.{}.{}\r\r{DESCRIPTION}",
    ...
    fn do_render(
        ...
        let full_settings: ZzzStrokeFullSettings = self.apply_settings(params)?;
    ...
    fn update_controls_disabled(
        params: &mut Parameters<ParamID>,
        descriptors: &[SettingDescriptor<ZzzStrokeFullSettings>],
        enabled: bool,
    ) -> Result<(), Error> {
    ...
    fn map_params(
        ...
        legacy_default_settings: &ZzzStrokeFullSettings,
    ) -> Result<(), Error> {
        fn get_defaults<T: example_effect::settings::SettingField + 'static>(
            defaults: &ZzzStrokeFullSettings,
            legacy_defaults: &ZzzStrokeFullSettings,
    ...
    fn apply_settings(
        &self,
        params: &mut Parameters<ParamID>,
    ) -> Result<ZzzStrokeFullSettings, Error> {
        let mut settings = ZzzStrokeFullSettings::default();

        fn apply_settings_list(
            descriptors: &[SettingDescriptor<ZzzStrokeFullSettings>],
            params: &mut Parameters<ParamID>,
            settings: &mut ZzzStrokeFullSettings,
        ) -> Result<(), Error> {
            for descriptor in descriptors {
                match &descriptor.kind {
                    SettingKind::Enumeration { options, .. } => {
