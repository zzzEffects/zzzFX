...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ZzzAmbientLightFullSettings>,
    strings: StringCache<ZzzAmbientLightFullSettings>,
    menu_item_strings: MenuItemCache<ZzzAmbientLightFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    ...
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"com.example:zzzAmbientLight".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ZzzAmbientLightFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let ps = su.property_suite.propSetString.ok_or(OfxStat::kOfxStatFailed)?;
    let defaults = ZzzAmbientLightFullSettings::default();
...
unsafe fn action_render(effect: OfxImageEffectHandle, inArgs: OfxPropertySetHandle) -> OfxResult<()> {
    ...
    let mut settings = ZzzAmbientLightFullSettings::default();
    apply_params(param_set, time, &mut settings)?;

    let ambient: ZzzAmbientLight = (&settings).into();
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut ZzzAmbientLightFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;
