...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ZzzLongShadowFullSettings>,
    strings: StringCache<ZzzLongShadowFullSettings>,
    menu_item_strings: MenuItemCache<ZzzLongShadowFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    ...
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"com.example:zzzLongShadow".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ZzzLongShadowFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = ZzzLongShadowFullSettings::default();
...
unsafe fn action_is_identity(
    ...
    let mut settings = ZzzLongShadowFullSettings::default();
    apply_params(param_set, time, &mut settings)?;
    let shadow: ZzzLongShadow = (&settings).into();
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut ZzzLongShadowFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;
