...
use zzzfx_core::svg_display::CachedSvg;
use zzzfx_core::{
    ZzzSvgDisplay, ZzzSvgDisplayFullSettings,
...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ZzzSvgDisplayFullSettings>,
    strings: StringCache<ZzzSvgDisplayFullSettings>,
    menu_item_strings: MenuItemCache<ZzzSvgDisplayFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    ...
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"com.example:zzzSvgDisplay".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ZzzSvgDisplayFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = ZzzSvgDisplayFullSettings::default();
...
unsafe fn action_render(
    ...
    let mut settings = ZzzSvgDisplayFullSettings::default();
    let (bg_r, bg_g, bg_b, bg_a, pos_x, pos_y) = apply_params(param_set, time, &mut settings)?;
    let core_settings: ZzzSvgDisplay = (&settings).into();
...
unsafe fn apply_params(
    param_set: OfxParamSetHandle,
    time: f64,
    dst: &mut ZzzSvgDisplayFullSettings,
    ...
    let find_id = |name: &str| -> OfxResult<SettingID<ZzzSvgDisplayFullSettings>> {
