...
use zzzfx_core::{
    ZzzCastShadow, ZzzCastShadowFullSettings,
...
const MANUAL_OFFSET_PARAM: &CStr = c"manual_center";
...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ZzzCastShadowFullSettings>,
    strings: StringCache<ZzzCastShadowFullSettings>,
    menu_item_strings: MenuItemCache<ZzzCastShadowFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    ...
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"com.example:zzzCastShadow".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ZzzCastShadowFullSettings>::new();
...
unsafe fn action_describe(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    ps(
        ...
        0,
        i18n::tr_cstr(TrKey::EffectCastShadowName).as_ptr(),
    )
    .ofx_ok()?;
    ps(
        ep,
    ...
    ps(
        ...
        0,
        i18n::tr_cstr(TrKey::EffectCastShadowDesc).as_ptr(),
    )
    .ofx_ok()?;
    ps(
        ep,
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = ZzzCastShadowFullSettings::default();
    ...
    // Native RGBA: Shadow Color (first)
    {
        ...
        ps(
            ...
            kOfxPropLabel.as_ptr(),
            0,
            i18n::tr_cstr(TrKey::NativeCastShadowColor).as_ptr(),
        )
        .ofx_ok()?;
        ps(
            pp,
            ...
            0,
            i18n::tr_cstr(TrKey::NativeCastShadowColorHint).as_ptr(),
        )
        .ofx_ok()?;
    ...
    // Native Double2D: Shadow Offset (second)
    {
        let mut pp: OfxPropertySetHandle = ptr::null_mut();
        pdef(
            param_set,
        ...
        ps(
            ...
            kOfxPropLabel.as_ptr(),
            0,
            i18n::tr_cstr(TrKey::NativeCastShadowOffset).as_ptr(),
        )
        .ofx_ok()?;
        ps(
            pp,
            kOfxParamPropHint.as_ptr(),
            0,
            i18n::tr_cstr(TrKey::NativeCastShadowOffsetHint).as_ptr(),
        )
        .ofx_ok()?;
    ...
    // Generic descriptors (native-mapped fields are skipped via is_native_grouped_name)
    for desc in d.settings_list.setting_descriptors.iter() {
        ...
        // Insert native Double2D "Manual Offset" after pivot_mode dropdown
        if desc.id.name == "pivot_mode" {
            let mut pp: OfxPropertySetHandle = ptr::null_mut();
            pdef(
                param_set,
                kOfxParamTypeDouble2D.as_ptr(),
                MANUAL_OFFSET_PARAM.as_ptr(),
                &mut pp,
            )
            .ofx_ok()?;
            ps(
                ...
                kOfxPropLabel.as_ptr(),
                0,
                i18n::tr_cstr(TrKey::NativeCastShadowManualOffset).as_ptr(),
            )
            .ofx_ok()?;
            ps(
                pp,
                kOfxParamPropHint.as_ptr(),
                0,
                i18n::tr_cstr(TrKey::NativeCastShadowManualOffsetHint).as_ptr(),
            )
            .ofx_ok()?;
            pd(pp, kOfxParamPropDefault.as_ptr(), 0, 0.5).ofx_ok()?;
            pd(pp, kOfxParamPropDefault.as_ptr(), 1, 0.5).ofx_ok()?;
            // Initially disabled (AutoSingle is default, not ManualSingle)
            (su.property_suite.propSetInt.ok_or(OfxStat::kOfxStatFailed)?)(
                pp, kOfxParamPropEnabled.as_ptr(), 0, 0,
            )
            .ofx_ok()?;
        }
    }
...
unsafe fn action_instance_changed(
    ...
    let pg_time = su.property_suite.propGetDouble.ok_or(OfxStat::kOfxStatFailed)?;

    // Get pivot_mode param handle and current value
    let mut param_set: OfxParamSetHandle = ptr::null_mut();
    gps(effect, &mut param_set).ofx_ok()?;
    let mut p: OfxParamHandle = ptr::null_mut();
    pgh(param_set, c"pivot_mode".as_ptr(), &mut p, ptr::null_mut()).ofx_ok()?;
    let mut time: OfxTime = 0.0;
    pg_time(inArgs, kOfxPropTime.as_ptr(), 0, &mut time).ofx_ok()?;
    let mut mode: c_int = 0;
    pgv(p, time, &mut mode).ofx_ok()?;

    // Get manual_center param property set
    let mut mo: OfxParamHandle = ptr::null_mut();
    pgh(param_set, MANUAL_OFFSET_PARAM.as_ptr(), &mut mo, ptr::null_mut()).ofx_ok()?;
    let mut pp: OfxPropertySetHandle = ptr::null_mut();
    su.parameter_suite
        .paramGetPropertySet
        .ok_or(OfxStat::kOfxStatFailed)?(mo, &mut pp)
        .ofx_ok()?;

    let enabled = mode == 2; // ManualSingle = 2
    (su.property_suite.propSetInt.ok_or(OfxStat::kOfxStatFailed)?)(
        pp, kOfxParamPropEnabled.as_ptr(), 0, if enabled { 1 } else { 0 },
    )
    .ofx_ok()?;
...
unsafe fn action_is_identity(
    ...
    let mut settings = ZzzCastShadowFullSettings::default();
    apply_params(param_set, time, &mut settings)?;
    let shadow: ZzzCastShadow = (&settings).into();
...
unsafe fn action_render(
    ...
    // Use DESTINATION bounds for the working region
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut ZzzCastShadowFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;
    let pgh = su.parameter_suite.paramGetHandle.ok_or(OfxStat::kOfxStatFailed)?;
    ...
    // Native Double2D: Manual Offset
    {
        ...
        let mut y: f64 = 0.0;
        pgv(p, time, &mut x, &mut y).ofx_ok()?;
        dst.manual_center_x = x as f32;
        dst.manual_center_y = y as f32;
    }
