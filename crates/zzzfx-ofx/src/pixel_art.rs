...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ZzzPixelArtFullSettings>,
    strings: StringCache<ZzzPixelArtFullSettings>,
    menu_item_strings: MenuItemCache<ZzzPixelArtFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    ...
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"com.example:zzzPixelArt".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ZzzPixelArtFullSettings>::new();
...
unsafe fn action_is_identity(
    ...
    let mut settings = ZzzPixelArtFullSettings::default();
    apply_params(param_set, time, &mut settings)?;
    let pixel_effect: ZzzPixelArt = (&settings).into();
...
unsafe fn action_render(
    ...
    let mut settings = ZzzPixelArtFullSettings::default();
    apply_params(param_set, time, &mut settings)?;
    let pixel_art: ZzzPixelArt = (&settings).into();
    ...
    RENDER_BUFS.with(|cell| {
        let do_render = |src_buf: &mut Vec<u8>, dst_buf: &mut Vec<u8>, tmp: &mut Vec<u8>| {
            ...
            if same_region {
                ...
                // Copy source into sub-rectangle; rest stays transparent (zero)
                src_buf.fill(0);
...
unsafe fn apply_params(
    ...
    time: f64,
    dst: &mut ZzzPixelArtFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;
    let pgh = su.parameter_suite.paramGetHandle.ok_or(OfxStat::kOfxStatFailed)?;
