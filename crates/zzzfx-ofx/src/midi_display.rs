...
struct EffectData {
    suites: SuiteCache,
    settings_list: SettingsList<ZzzMidiDisplayFullSettings>,
    strings: StringCache<ZzzMidiDisplayFullSettings>,
    menu_item_strings: MenuItemCache<ZzzMidiDisplayFullSettings>,
}
...
pub fn get_plugin() -> *const OfxPlugin {
    ...
    let pi = PLUGIN_INFO.get_or_init(|| OfxPlugin {
        ...
        apiVersion: 1,
        pluginIdentifier: c"com.example:zzzMidiDisplay".as_ptr(),
...
unsafe fn set_host_info_inner(host: *mut OfxHost) -> OfxResult<()> {
    ...
    let settings_list = SettingsList::<ZzzMidiDisplayFullSettings>::new();
...
unsafe fn action_describe_in_context(desc: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    let defaults = ZzzMidiDisplayFullSettings::default();
...
unsafe fn action_render(
    ...
    let mut settings = ZzzMidiDisplayFullSettings::default();
...
unsafe fn apply_params(
    param_set: OfxParamSetHandle,
    time: f64,
    dst: &mut ZzzMidiDisplayFullSettings,
) -> OfxResult<()> {
    let d = data()?;
    let su = &d.suites;
    ...
    let find_id = |name: &str| -> OfxResult<SettingID<ZzzMidiDisplayFullSettings>> {
