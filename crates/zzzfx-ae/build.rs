...
fn main() {
    if std::env::var_os("CARGO_CFG_WINDOWS").is_some() || std::env::var_os("CARGO_CFG_TARGET_OS").as_deref() == Some(OsStr::new("macos")) {
        ...
        let (name, category, match_name, entry_name) = if cfg!(feature = "effect-stroke") {
            ("zzzFX Stroke", "zzzFX", "zzzfx-stroke", "ZZZFXStrokeMain")
        } else if cfg!(feature = "effect-repeater") {
            ("zzzFX Repeater", "zzzFX", "zzzfx-repeater", "ZZZFXRepeaterMain")
        } else if cfg!(feature = "effect-sprite-sheet") {
            ("zzzFX Sprite Sheet", "zzzFX", "zzzfx-sprite-sheet", "ZZZFSSpriteSheetMain")
        } else {
            ("zzzFX Stroke", "zzzFX", "zzzfx-stroke", "ZZZFXStrokeMain")
        };
        ...
        pipl::plugin_build(vec![
            ...
            #[cfg(target_os = "macos")]
            Property::CodeMacIntel64(entry_name),
            #[cfg(target_os = "macos")]
            Property::CodeMacARM64(entry_name),
